//
//  ChangeVersionViewController.swift
//  KingOfStrenth
//
//  Created by 琳琳 on 16/10/10.
...
class ChangeVersionViewController: BaseViewController{
...
//            }
    ...
    func layoutPageSubViews() {
        ...
        mathLabel.snp_makeConstraints { (make) in
            ...
            make.right.equalTo(bgView).offset(-90)
            make.height.equalTo(30)
        }
    ...
    func configBooks() {
        ...
        if self.phase == "2" && dataSource != nil {
            for i in [0,1,2,4,5] {
                ...
                item.subType = String(currentData![subType!][0]["jiaocai_type"].stringValue)
                item.id = String(currentData![subType!][0]["id"].stringValue)
                item.bookName = currentData![subType!][0]["book_name"].stringValue
                selectInfo[i] = item
    ...
    var saveButton: UIButton {
        if _saveButton == nil {
            _saveButton = UIButton(type: .Custom)
            _saveButton.setImage(UIImage(named: "Setting_save_btn_normal_iPhone"), forState: .Normal)
            _saveButton.setImage(UIImage(named: "Setting_save_btn_selected_iPhone"), forState: .Selected)
            _saveButton.addTarget(self, action: #selector(ChangeVersionViewController.save(_:)), forControlEvents: .TouchUpInside)
        }
        return _saveButton
    }
...
extension ChangeVersionViewController: UIPickerViewDelegate,UIPickerViewDataSource,CourseViewCallBackDelegate {
    ...
    func callBackSuccess(manager: CSAPIBaseManager) {
        ...
        if manager.isKindOfClass(SaveVersionManager) {
            YAlertViewController.showAlertController(self, title: "提示", message: "保存成功")
            return
        }
    }
